...
export default async function handler(req, res) {
  ...
  let classificacao = [];
  if (seasonId){
    const standingsData = await safeJson(`${BASE}/leagues/${id}/standings/?season_id=${seasonId}`, headers);
  ...
  return res.status(200).json({
    ...
    season: seasonInfo ? { nome: seasonInfo.name, inicio: seasonInfo.start_date, fim: seasonInfo.end_date } : null,
    classificacao, estatisticas, proximoJogo,
    _debugSeasonInfo: seasonInfo
  });
}
