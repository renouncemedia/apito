...
export default async function handler(req, res) {
  ...
  const [recentesRaw, proximosRaw] = await Promise.all([
    safeJson(`${BASE}/events/?team_id=${id}&status=finished&limit=30`, headers),
    safeJson(`${BASE}/events/?team_id=${id}&status=notstarted&limit=30`, headers)
  ]);

  function mapaJogos(raw){
    const lista = raw?.results || [];
    return lista.map(e => ({
      id: e.id, data: e.event_date, ligaId: e.league_id, ligaNome: e.league_name || null,
      jornada: e.round_number ?? null,
