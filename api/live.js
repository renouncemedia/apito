...
// odds: sub-recurso documentado /events/{id}/odds/ -> { event_id, odds: { home_win, draw, away_win, ... } }
async function oddsDoJogo(id, headers){
  try {
    const r = await fetch(`${BASE}/events/${id}/odds/`, { headers });
    if (!r.ok) return null;
    const d = await r.json();
    const o = d.odds || d;
    if (o && (o.home_win || o.draw || o.away_win)){
      return { casa: o.home_win ?? null, empate: o.draw ?? null, fora: o.away_win ?? null };
    }
    return null;
  } catch (e) { return null; }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // odds só para os primeiros 40 jogos, para não sobrecarregar o pedido
    const listaComOdds = lista.slice(0, 40);
    const mapaOdds = {};
    await Promise.all(listaComOdds.map(async e => { mapaOdds[e.id] = await oddsDoJogo(e.id, headers); }));
