  ...
  try {
    ...
    function mapTime(t){
      ...
      return {
        ...
        titulares: (t.players || []).map(p => ({ id: p.id, nome: p.name, numero: p.jersey_number, posicao: p.position, capitao: !!p.captain })),
    ...
    if (statsHome && statsAway){
      ...
      if (typeof statsHome.xg?.actual === 'number' && typeof statsAway.xg?.actual === 'number'){
    ...
    const shotmap = Array.isArray(statsResp?.shotmap) ? statsResp.shotmap.map(s => ({
      x: s.pos?.x, y: s.pos?.y, tipo: s.type, ehCasa: s.home, minuto: s.min, xg: s.xg
    })).filter(s => s.x !== undefined && s.y !== undefined) : [];

    res.setHeader('Cache-Control', 's-maxage=20, stale-while-revalidate=40');
    return res.status(200).json({ jogo, eventos, escalacoes, lineupsStatus, h2h, emFalta, estatisticas, momentum, posicoesMedias, shotmap });
